import { wid, wid2xy } from "../app/constant"
import { Img } from "../app/utils/img"
import type { MemMapTile } from "./api/apiModel"
...
export class MapTile {
  ...
  draw(ctx: CanvasRenderingContext2D, cfg: MapCtx) {
    ...
    if (cfg.showLv && this.data.tileType != 0) { // 绘制等级
      ...
      const img = this.mapCtx.getSg().ctx.res.getImg(`playing#comm#level_${this.data.curLevel}`)
      if (img) {
