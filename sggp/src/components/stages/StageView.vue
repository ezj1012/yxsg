...
watch(sg.ctx.stageMgr.stageKey, async (newStageKey, oldStageKey) => {
    ...
    oldStage && oldStage.onUnmounted && await oldStage.onUnmounted()
    newStage && newStage.onMounted && await newStage.onMounted()
...
onUnmounted(async () => {
    stage.value && stage.value.onUnmounted && await stage.value.onUnmounted()
})
</script>
<template>
