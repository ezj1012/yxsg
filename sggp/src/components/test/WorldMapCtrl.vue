...
actionMgr.reg('WM_MoveTo', async (sg) => {
    await moveTo(x.value, y.value)
})
actionMgr.reg('WM_MoveCity', async ({ sg }) => {
    const cityId = sg.dataMgr.getByKey('playing#player_lastCity')
    const xy = cid2xy(cityId)
    await moveTo(xy.x, xy.y)
})
actionMgr.reg('WM_MoveUp', async ({ sg }) => { await moveTo(Number(x.value) - 4, Number(y.value) - 4) })

actionMgr.reg('WM_MoveUpRight', async ({ sg }) => { await moveTo(Number(x.value), Number(y.value) - 5) })
actionMgr.reg('WM_MoveUpLeft', async ({ sg }) => { await moveTo(Number(x.value) - 5, Number(y.value)) })

actionMgr.reg('WM_MoveRight', async ({ sg }) => { await moveTo(Number(x.value) + 3, Number(y.value) - 3) })
actionMgr.reg('WM_MoveDownRight', async ({ sg }) => { await moveTo(Number(x.value) + 5, Number(y.value)) })

actionMgr.reg('WM_MoveDown', async ({ sg }) => { await moveTo(Number(x.value) + 4, Number(y.value) + 4) })
actionMgr.reg('WM_MoveDownLeft', async ({ sg }) => { await moveTo(Number(x.value), Number(y.value) + 5) })
actionMgr.reg('WM_MoveLeft', async ({ sg }) => { await moveTo(Number(x.value) - 3, Number(y.value) + 3) })
