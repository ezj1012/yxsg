        ...
        mounted(el, binding) {
            ...
            const inTestFun = (e: MouseEvent) => {
                if (msg.isHover ? msg.isHover(e) : true) {
                    ...
                    sg.value?.ctx.hoverMsgMgr.setHoverMsg(el, { clientX, clientY, el, content: msg.content })
                } else {
                    outFun(e)
                }
            }
            const outFun = (e: MouseEvent) => {
                ...
                hover = false
                sg.value?.ctx.hoverMsgMgr.setHoverMsg(el)
            }
            let hover = false
...
export namespace DivBg {
    ...
    function setSize(el: HTMLDivElement, binding: DirectiveBinding<any>, sg: ShallowRef<SanGuo | undefined>) {
        ...
        if (binding.value.size) {
            const size = binding.value.size as Shapable
    ...
    function setBg(el: HTMLDivElement, binding: DirectiveBinding<any>, sg: ShallowRef<SanGuo | undefined>) {
        ...
        let img: Img.ImgDef
        if (typeof binding.value === 'string') {
            img = sg.value.ctx.res.getImg(binding.value as string)!
